...
function PreviewImageCard({ isLight, darkImg, lightImg, alt, isHero = false }) {
  return (
    <div className="relative flex min-h-[460px] items-center justify-center lg:-mr-16">
      {!isLight && (
        <>
          <div className="absolute right-10 top-1/2 h-[380px] w-[520px] -translate-y-1/2 rounded-full bg-blue-600/20 blur-[100px]" />
          <div className="absolute bottom-10 right-20 h-[220px] w-[350px] rounded-full bg-emerald-400/10 blur-[90px]" />
        </>
      )}

      <div className="relative w-full max-w-[850px]">
        {!isLight && (
          <div className="absolute -inset-5 rounded-[40px] bg-indigo-500/15 blur-3xl" />
        )}

        <img
          src={isLight ? lightImg : darkImg}
          alt={alt}
          className={`relative z-10 w-full object-contain ${
            isHero ? "hero-image-motion" : ""
          } ${
            isLight
        ...
        {!isLight && (
          <>
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 h-24 bg-gradient-to-t from-[#050b1d] via-[#050b1d]/60 to-transparent" />
            <div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-24 bg-gradient-to-r from-[#050b1d] to-transparent" />
            <div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-16 bg-gradient-to-l from-[#050b1d] to-transparent" />
          </>
        )}
      </div>
    </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center border border-dashed border-white/15 bg-gradient-to-br from-[#0b1730] to-[#071229]">
          <div className="px-6 text-center">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Replace <span className="text-slate-300">img: null</span> in phase data
            </p>
          </div>
        </div>
      )}
...
const landingThemeStyles = `
  .landing-page { transition: background-color .35s ease, color .35s ease; }
  html, .landing-page { scroll-behavior: smooth; }
  .landing-page section { scroll-margin-top: 80px; }
  .landing-page button, .landing-page a, .landing-page article, .landing-page img {
    transition: transform .3s cubic-bezier(.22,1,.36,1), opacity .3s ease, background-color .3s ease, border-color .3s ease, color .3s ease, box-shadow .3s ease;
  }
  .landing-page button:hover { transform: translateY(-2px); }
  .landing-page button:active { transform: translateY(0) scale(.98); }
  .landing-page article:hover { transform: translateY(-4px); box-shadow: 0 24px 65px rgba(0,0,0,.16); }
  .landing-page article:hover img { transform: scale(1.025); }
  ...
  .landing-page .reveal-up { transform: translateY(28px); }
  .landing-page .reveal-left { transform: translateX(-90px); }
  .landing-page .reveal-right { transform: translateX(90px); }
  .landing-page .reveal-on-scroll.is-visible { opacity: 1; transform: translate3d(0, 0, 0); }
  .landing-page .hero-entrance { animation: heroEntrance .9s cubic-bezier(.22,1,.36,1) both; }
  .landing-page .hero-image-motion { animation: heroImageMotion 7s ease-in-out 1s infinite; }
  @keyframes heroEntrance { from { opacity: 0; transform: translateY(22px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes heroImageMotion { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-7px); } }
  .status-marquee { background: linear-gradient(90deg, #062f2a, #075e54, #0b7a68, #075e54, #062f2a); border-color: rgba(52, 211, 153, 0.25); }
  .status-marquee .marquee-track { animation: marqueeRight 24s linear infinite; will-change: transform; }
  .status-marquee span { color: #d1fae5; }
  .status-marquee svg { color: #6ee7b7; }
  @keyframes marqueeRight { from { transform: translateX(-50%); } to { transform: translateX(0); } }
  .landing-light .status-marquee { background: linear-gradient(90deg, #ecfdf5, #d1fae5, #a7f3d0, #d1fae5, #ecfdf5) !important; border-color: #a7f3d0 !important; }
  .landing-light .status-marquee span { color: #047857 !important; }
  .landing-light .status-marquee svg { color: #059669 !important; }
  .landing-page.landing-dark { background: #050b1d; color: #ffffff; }
  .landing-page.landing-light { background: #ffffff; color: #0f2942; }
`;
...
export default function SwasthyaSakhaLanding({
  ...
  return (
    <>
      ...
      <main
        className={`landing-page min-h-screen overflow-hidden text-white ${
            ...
            <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:py-28">
              <div className="grid items-center gap-12 lg:grid-cols-[.75fr_1.25fr]">
                ...
                <PreviewImageCard
                  isLight={isLight}
                  darkImg={image2}
                  lightImg={image2Light}
                  alt="Swasthya Sakha architecture map"
                />
              </div>
            </div>
          </Reveal>
          ...
          <div className="marquee-track flex w-max items-center gap-10 py-4 text-xs font-bold uppercase tracking-[0.18em]">
            ...
            <div className="flex items-center gap-10">
              {marqueeItems.map((x, idx) => (
                <span key={`dup-${x}-${idx}`} className="flex items-center whitespace-nowrap">
                  {x}
                  <BarChart3 className="ml-8" size={16} />
                </span>
              ))}
        ...
        <footer className="border-t border-white/10">
          <div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-10 sm:px-8 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-xs text-slate-500">By Nexvion</p>
            </div>
          </div>
        </footer>
      </main>
